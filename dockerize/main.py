...
def parse_args():
    ...
    group.add_argument('--tag', '-t', help='Tag to apply to Docker image')
    group.add_argument('--cmd', '-c')
    group.add_argument('--entrypoint', '-e')

    group = parser.add_argument_group('Output options')
    group.add_argument(
        '--no-build', '-n', action='store_true', help='Do not build Docker image'
    )
    ...
    parser.add_argument(
        '--env', action='append', default=[], help='Add ENV={arg} in Dockerfile'
    )
    parser.add_argument(
        '--workdir', default=None, help='Add WORKDIR={arg} in Dockerfile'
    )
    ...
    parser.add_argument(
        '--filetools', action='store_true', help='Add common file manipulation tools'
    )

    group = parser.add_argument_group('Container options')
    parser.add_argument(
        '--runtime', '-R', help='Set container engine for building', default='docker'
    )
    parser.add_argument(
        '--buildcmd', '-B', help='Set command for building', default='build'
    )

    group = parser.add_argument_group('Logging options')
    group.add_argument(
        '--verbose', action='store_const', const=logging.INFO, dest='loglevel'
    )
    group.add_argument(
        '--debug', action='store_const', const=logging.DEBUG, dest='loglevel'
    )
...
def main():
    ...
    try:
        args.symlinks = getattr(SymlinkOptions, args.symlinks.upper().replace('-', '_'))
